...
namespace PsdUIExporter {
    ...
    public static class ExportUtility {
        ...
        [MenuItem("Restar工具/PsdTools/PSDWindow")]
        ...
        [MenuItem("Restar工具/PsdTools/SliceWindow")]
        ...
        public static Rect GetRectFromLayer(IPsdLayer psdLayer, INode parentNode) {
            ...
            // Debug.Log($"left: {left}, bottom: {bottom}, top: {top}, right: {rigtht}");
            // Debug.Log($"width: {width}, height: {height}");

            // var xMin = (rigtht + left - parentRect.width) * 0.5f;
            // var yMin = -(top + bottom - parentRect.height) * 0.5f;
            // Debug.Log($"parentNode: rect X: {parentNode.GetRect().x}, Y: {parentNode.GetRect().y} width: {parentNode.GetRect().width}, height: {parentNode.GetRect().height}");
            Vector2 pa = GetParenRectAddition(parentNode);
            var xMin = (left + width / 2f) - pa.x;
            var yMin = pa.y - (top + height / 2f);

            // Debug.Log($"xMin: {xMin}, yMin: {yMin}, width: {width}, height: {height}");
            // Debug.Log($"pa: {pa}");
        ...
        public static void SetPlatformTextureSettings(TextureImporter importer, int maxSize = 2048, int type = COMPRESS_ALPHA) {
            ...
            switch (type) {
                case SPLIT_ALPHA:
                    ...
                    importer.isReadable = true;  // 这个只是打包用到
